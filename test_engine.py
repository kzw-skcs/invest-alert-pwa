...
print("[サイクル補正(v3.27.1: ディフェンシブは完全ゼロ)]")
...
d_sea, _ = E.value_cycle_delta(9, 2026, "inflow", defensive=True)
check("流入+3(通常)", d_in == 3)
check("ディフェンシブは常にゼロ(流入)", d_ind == 0 and any("純チャート判定" in x for x in f_ind))
check("ディフェンシブは常にゼロ(流出)", d_out == 0)
check("ディフェンシブは常にゼロ(季節+選挙)", d_sea == 0)
check("上限±8(通常銘柄)", E.value_cycle_delta(9, 2026, "inflow")[0] == 8)
